...
.at-inline .at-label{margin: 10px 12px !important;}
.at-label{float:left;clear:left;}
.at-inline .at-text{width: 70px;float:left;}
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_url( $field, $meta) {  
    $this->show_field_begin( $field, $meta );
    echo "<input type='url' class='at-text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' style='{$field['style']}' size='50' />";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_number( $field, $meta) {  
    $this->show_field_begin( $field, $meta );
    echo "<input type='number' class='at-text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' style='{$field['style']}' size='50' />";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_wysiwyg( $field, $meta ,$in_repeater = false) {
    ...
    if ( version_compare( $wp_version, '3.2.1' ) < 1 || $in_repeater) {
      echo "<textarea style='{$field['style']}' class='at-wysiwyg theEditor large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='4'>{$meta}</textarea>";
    }else{
      // Use new wp_editor() since WP 3.3
      wp_editor( stripslashes(html_entity_decode($meta)), $field['id'], array( 'editor_class' => 'at-wysiwyg','textarea_rows' => 4 ) );
    }
    $this->show_field_end( $field, $meta );
  }
   ...
   *  @author Ohad Raz | edited by Shon L. Burns
   *  @since 1.0
   *  @access public
   *  @param $id string  field id, i.e. the meta key
   *  @param $args mixed|array
  ...
  public function addNumber($id,$args,$repeater=false){
    $new_field = array('type' => 'number','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'URL Field','multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  public function addWysiwyg($id,$args,$repeater=false){
    $new_field = array('type' => 'wysiwyg','id'=> $id,'std' => '','desc' => '','style' =>'width: 300px; height: 200px','name' => 'WYSIWYG Editor Field','multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
